...
def train_client(dataloader_for_client, test_loader, epoch, index, mode, local_rounds=20, num_clients=20, lr=0.001):
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    i = index
    ...
    # train client model
    desc = f'Epoch {epoch}, Client {i+1}, Local Round'
    for round in tqdm.tqdm(range(local_rounds), desc=desc, colour='yellow'):
        ...
        with open (os.path.join(CLIENT_LOG_PATH, f'results_{num_clients}_{mode}_client{i+1}_multi.txt'), 'a') as f:
...
def train(num_epoch, mode='all', local_rounds=20, num_clients=20, batch_size=32, lr=0.001):
    ...
    if( mode == 'all'):
        ...
        index = np.random.choice(20, num_clients, replace=False)
    
    # write the index to the file
    with open(f'./results_{num_clients}_{mode}_multi.txt', 'a') as f:
        f.write(f'Index: {index}\n')

    for epoch in tqdm.tqdm(range(num_epoch), desc='Epoch', colour='blue'):

        # train clients
        for i in tqdm.tqdm(range(num_clients), desc='Client', colour='green'):
            p = Process(target=train_client, args=(dataloader_for_client, test_loader, epoch, index[i], mode, local_rounds, num_clients, lr))
            p.start()
            processes.append(p)
...
def main():
    ...
    # 读取用户选择的阶段
    stage = input('Please input the stage you want to run: ')
    # 读取参数
    num_epoch = int(input('Please input the number of epochs: '))
    mode = input('Please input the mode (all or partial): ')
    local_rounds = int(input('Please input the number of local rounds: '))
    num_clients = int(input('Please input the number of clients: '))
    ...
    if (stage == '1'):
        ...
        train(num_epoch=num_epoch, mode=mode, local_rounds=local_rounds, num_clients=num_clients, batch_size=batch_size, lr=lr)
    else:
        # stage 2 activate 10 clients
        print('Stage 2: Training 15 clients randomly')
        num_input = input('Please input the number of clients: ')
        train(num_epoch=num_epoch, mode=mode, local_rounds=local_rounds, num_clients=num_clients, batch_size=batch_size, lr=lr)
